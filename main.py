...
from config import TOKEN, CATEGORIA_FARM_ID, ID_MARCADOR, CANAL_REGISTRO_ID, CANAL_LOG_ID, CANAL_PEDIDO_ID
from views.farmview import FarmView
from views.registro import enviar_botao_registro, RegistroView, verificar_registro_apagado, AvaliacaoRegistroView
...
async def task_limpar_prints():
    try:
        ...
        if removidos:
            print(f"🧹 Limpeza: {removidos} print(s) removido(s).")
...
async def on_ready():
    ...
    try:
        ...
        bot.add_view(AvaliacaoView(None, None, None))
        ...
        if canal_pedido:
            ...
            if not ja_existe:
                ...
                try:
                    await _recriar_botao_pedido(canal_pedido, bot)
                finally:
...
async def on_guild_channel_create(canal):
    if not (
        isinstance(canal, discord.TextChannel)
        and canal.category_id == CATEGORIA_FARM_ID
        and canal.id != CANAL_PEDIDO_ID
    ):
        return
    bot._suppress_recreate_farm.add(canal.id)
    try:
...
async def on_message_delete(message: discord.Message):
    ...
    elif conteudo == "__REGISTRO_BOT_MARKER__":
        if message.channel.id in getattr(bot, "_suppress_recreate_registro", set()):
            return
        await enviar_botao_registro(bot)

    elif conteudo == "__PEDIDO_BOT_MARKER__":
...
async def on_member_join(member):
    ...
    if canal_registro:
        try:
            await member.send(
                f"👋 Seja bem-vindo, {member.display_name}!\n"
                f"Vá até <#{canal_registro.id}> e clique no botão para se registrar."
            )
